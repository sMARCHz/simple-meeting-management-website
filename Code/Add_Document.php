...
<body>
	...
	<center>
		<div class="nav" id="bar">
			...
			<a class="active" href="Add_remove_Document.php"> Add/Remove Document </a>
...
<body>
	...
	FROM `AllMeeting` WHERE CURRENT_DATE()>AllMeeting.Date; ";
		...
		echo "<form  action=\"Add_Document.php\" method=\"post\" >
		<table id=\"r\"><tr>
			...
			while ($row = mysqli_fetch_assoc($resultSelect2)) {
				...
				if ($link == null) {
					echo "<tr>
						...
						<td class=\"red\"> 
						<input name=\"txtDocument[]\" type=\"text\" id=\"txtDocument[]\"style=\"width:7vw\"></a></td>
						<td class=\"red\"> <a  href=\"$link\" target=\"_blank\" >" . $row["Conclusion"] . "</a></td>
...
<td>" . $row["Topic"] . "</td><td>" . $row["Date"] . "</td><td>" . $row["Time"] . "</td><td>" . $row["Place"] . "</td><td>" . $row["Host"] . "</td><td> 
<input name=\"txtDocument1[]\" type=\"text\" id=\"txtDocument1[]\"style=\"width:7vw\"></a></td>
<td> <a  href=\"$link\" target=\"_blank\" >" . $row["Conclusion"] . "</a></td>	<td >Ongoing..</td></tr>";
				}
			}
			echo "</table>";
		} else {
    ...
    Date,Time,Place,Host,Doc_Link,Conclusion FROM AllMeeting WHERE CURRENT_DATE()>AllMeeting.Date; ";
		...
		if (mysqli_num_rows($resultSelect10) > 0) {
			while ($row = mysqli_fetch_assoc($resultSelect10)) {
				$link = $row["Doc_Link"];
				if ($link == null) {
					array_push($idArray, $row["MeetingID"]);
				}
			}
		}
		for ($i = 0; $i < count($idArray); $i++) {
			if ($_POST["txtDocument"][$i] != null) {
				$sqlSelect11 = "UPDATE AllMeeting SET`Doc_Link`= '" . $_POST["txtDocument"][$i] . "' WHERE MeetingID='" . $idArray[$i] . "'";
				$resultSelect11 = mysqli_query($conn, $sqlSelect11);
			}
			if ($resultSelect11)
				echo "<head><meta http-equiv='Refresh'content = '1; URL =Add_Document.php'></head>";
		}
    ...
    Date,Time,Place,Host,Doc_Link,Conclusion FROM AllMeeting WHERE CURRENT_DATE()<=AllMeeting.Date; ";
		...
		if (mysqli_num_rows($resultSelect20) > 0) {
			while ($row = mysqli_fetch_assoc($resultSelect20)) {
				$link = $row["Doc_Link"];
				if ($link == null) {
					array_push($idArray1, $row["MeetingID"]);
				}
			}
		}
		for ($j = 0; $j < count($idArray1); $j++) {
			if ($_POST["txtDocument1"][$j] != null) {
				$sqlSelect21 = "UPDATE AllMeeting SET`Doc_Link`= '" . $_POST["txtDocument1"][$j] . "' WHERE MeetingID='" . $idArray1[$j] . "'";
