...
<head>
	...
	<script language="JavaScript">
		function toggle(source) {
			checkboxes = document.getElementsByName('meeting');
...
<body>
	...
	FROM `AllMeeting` WHERE CURRENT_DATE()>AllMeeting.Date; ";
		...
		echo "<table id=\"r\" ><tr><th>Topic</th><th>Date</th><th>Time</th><th>Place</th><th>Host</th><th>Document</th><th>Conclusion</th><th>Status</th>";
		if (mysqli_num_rows($resultSelect) > 0) {
			...
			while ($row = mysqli_fetch_assoc($resultSelect2)) {
				...
				echo "<tr >
					<td class=\"red\" ><a  style=\" color: black;\" href=\"conference_userdata.php?variable=$ID\">" . $string . "</a></td>
			...
			while ($row = mysqli_fetch_assoc($resultSelect)) {
				...
				$linkc = $row["Conclusion"];
				echo "<tr >
			<td ><a  style=\" color: black;\" href=\"conference_userdata.php?variable=$ID\">" . $string . "</a></td>
			<td >" . $row["Date"] . "</td>
			<td >" . $row["Time"] . "</td>
			<td  data-role=\"popover\" data-popover-text=" . $row["Place"] . "  data-popover-position=\"bottom\" data-popover-hide=\"1000\" >" . $row["Place"] . "</td>
			<td >" . $row["Host"] . "</td>
			<td ><a  href=\"$link\" target=\"_blank\" >" . $row["Doc_Link"] . "</a></td>
			<td ><a  href=\"$linkc\" target=\"_blank\" >" . $row["Conclusion"] . "</a></td>
			<td >Ongoing..</td></tr>";
			}
			echo "</table>";
		} else {
